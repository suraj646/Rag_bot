...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=EGM9UUZHOLBI148K"
    r = requests.get(url)
    return r.json()
...
def chat(state: state, config=None):
    ...
    response = tool_model.invoke(messages, config=config)
...
bot=graph.compile(checkpointer=checkpoint)
